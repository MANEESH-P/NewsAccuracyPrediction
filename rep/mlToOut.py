...
DEFAULTSFILEPATH = 'F:/flask_project/rep/reputationDict.csv'
NORMALFILEPATH = 'F:/flask_project/rep/reputations.csv'
...
class source:
    ...
    def addArticle(self, articleId, articleValidity):
        if not str(articleId) in self.articles:
            self.reputation = float(self.reputation)
            self.reputation = (self.reputation*self.size +
                               articleValidity)/(self.size+1)
    ...
    for op in opinions:
        ...
        if op.stance == 1:
            if op.sourceName in globals.sources:
                globals.sources.get(op.sourceName).reputation = float(
                    globals.sources.get(op.sourceName).reputation)
        ...
        if op.sourceName in globals.sources:
            ...
            if op.stance == 0:
                if opinion.stance == 0:
                    globals.sources.get(op.sourceName).reputation = float(
                        globals.sources.get(op.sourceName).reputation)
                    finalStance += globals.sources.get(
                        op.sourceName).reputation
...
def updateRep(opinions):
    for op in opinions:
        if not op.sourceName in globals.sources:
            ...
            globals.sources.update(
                {op.sourceName: source(op.sourceName, 0, [], 1)})
        globals.sources.get(op.sourceName).addArticle(
            op.articleId, compareStance(op, opinions))
...
def loadReputations(filepath):
    with open(filepath) as csvfile:
        ...
        for row in reader:
            globals.sources[row['source']] = source(
                row['source'], row['reputation'], row['articles'], row['size'])
            globals.sources[row['source']].articles = []
            globals.sources[row['source']].size = 100  # Only for defaults


def loadDefaultRepsFromDisk(filepath):
    print("loading default reps from disk")
    with open(filepath) as csvfile:
    ...
    with open(filepath, 'w') as csvfile:
        ...
        for k, v in globals.sources.items():
            if(type(v) == source):
                print("reputation: ", v.reputation)
                writer.writerow(
                    {'source': k, 'reputation': v.reputation, 'articles': v.articles, 'size': v.size})
            else:
                print(source)
